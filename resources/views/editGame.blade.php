...
<div style="border: 3px solid;">
    <form action="/edit-game/{{$game->id}}" method="POST">
        @csrf
        @method('PUT')
        <input name = "title" type="text" placeholder="Game title" value = "{{$game->title}}" required>
        ...
        <textarea name="popis" type="text" placeholder="nieco o hre" required>{{$game->Popis}}</textarea> {{--//normalne by som sa zabil za toto velke pismeno na zaciatku 20 min debugu--}}
